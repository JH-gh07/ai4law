...
_MODULE_LEGAL_SOURCE_MAP: dict[str, dict[str, list[dict[str, Any]]]] = {
    ...
    "tia": {
        "transfer_tool": [
            {"title": "GDPR", "article": "46", "weight": 1.0},
            {"title": "GDPR", "article": "44", "weight": 0.85},
        ],
        "country_risk": [
    ...
    "eu_scc": {
        "documentation": [
            {"title": "EU 2021/914", "article": "", "weight": 0.90},
            {"title": "GDPR", "article": "46", "weight": 0.80},
        ],
        "contract": [
            {"title": "EU 2021/914", "article": "", "weight": 1.0},
            {"title": "GDPR", "article": "46", "weight": 0.80},
        ],
        "cross_border": [
        ...
        "other": [
            ...
            {"title": "GDPR", "article": "46", "weight": 0.70},
        ],
    },
}
...
def _extract_article_number(article: str, jurisdiction: str) -> str:
    ...
    if jurisdiction.upper() == "EU":
        ...
        if match:
            return match.group(1)
        match = re.search(r"\bClause\s+([0-9]+(?:\.[0-9]+)*)", text, re.I)
        if match:
            return match.group(1)
        match = re.search(r"\bAnnex\s+([A-Z0-9]+)\b", text, re.I)
        if match:
...
def _infer_authority(title: str, source_id: str) -> str:
    ...
    if "GDPR" in upper:
        return "high"
    if "EU 2021/914" in upper or "SCC" in upper:
        return "high"
    if "EDPB" in upper or "RECOMMENDATION" in upper:
        return "medium"
    return "medium"
...
def _display_label(title: str, article_no: str, source_id: str, raw_article: str) -> str:
    ...
    upper = (title or "").upper()
    if "GDPR" in upper:
        return f"GDPR {raw_article or f'Article {article_no}'}".strip()
    if "EU 2021/914" in upper or "SCC" in upper:
        return f"{title} {raw_article}".strip() if raw_article else title
    if "EDPB" in upper and raw_article:
        return f"{title} {raw_article}".strip()
    return f"{title} {raw_article}".strip() if raw_article else title
