...
def _fast_render(task_id, payload, chapters, gaps, attachment_notes):
    return {
...
def test_cpra_generate_report_uses_enhanced_attachment_facts(monkeypatch) -> None:
    ...
    def _fake_render(task_id, payload, chapters, gaps, attachment_notes):
        captured["payload"] = payload
        captured["gaps"] = gaps
        return _fast_render(task_id, payload, chapters, gaps, attachment_notes)
...
def test_cpra_generate_report_adds_consistency_review_issues(monkeypatch) -> None:
    ...
    service.extractor.extract = _extractor_stub
    service._render = _fast_render
    service._generate_chapters_from_context = lambda context_block, level, citations: [
        CPRAChapter(
            chapter_no=1,
            title="执行摘要",
...
def test_cpra_generate_report_prefers_gap_level_citations(monkeypatch) -> None:
    ...
    service.legal_retriever.retrieve_for_gaps = lambda gaps: {
        service.legal_retriever._gap_key(gap, idx): [  # noqa: SLF001
            {"source": f"GAP:{gap.domain}:{idx}", "snippet": "gap-specific"}
        ]
        for idx, gap in enumerate(gaps)
    }
    service.legal_retriever.retrieve = lambda domain, extra_context="": [
        {"source": f"DOMAIN:{domain}", "snippet": "domain-level"}
    ]
    ...
    assert any(citation.startswith("GAP:") for item in result.gap_items for citation in item.citations)
