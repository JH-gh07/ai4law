"""Schema-first BCR review adapter tests."""
...
def _chapters() -> list[BCRChapter]:
    return [
        BCRChapter(chapter_no=1, title="BCR 类型判断", content="本BCR遵循GDPR第47条 [1]。", risk_level="HIGH"),
        BCRChapter(chapter_no=2, title="整改建议", content="补充数据主体权利机制说明。", risk_level="LOW"),
    ]


def test_bcr_document_ir_matches_golden_snapshot() -> None:
    doc, rep = build_bcr_document_ir(
...
def test_production_footnote_shape_yields_claim_block() -> None:
    doc, rep = build_bcr_document_ir(
        ...
        chapters=[BCRChapter(chapter_no=1, title="T", content="法规依据 [1]。", risk_level="HIGH")],
