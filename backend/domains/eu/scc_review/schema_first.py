"""Schema-first adapter for the SCC review chapter output."""
...
def build_scc_document_ir(
    *,
    ...
    """Translate SCC review chapter output into typed DocumentIR."""
    ...
    for chapter in chapters:
        ...
        for index, paragraph in enumerate(paragraphs, start=1):
            ...
            if not text:
                continue
            block_id = f"scc.chapter.{chapter.chapter_no}.block.{index}"
            if citation_refs:
                registered = all(
        ...
        sections.append(SectionIR(
            section_id=f"scc.chapter.{chapter.chapter_no}",
            title=chapter.title,
            level=2,
    ...
    document = DocumentIR(
        ...
        prompt_version="legacy-scc-adapter-v1",
        template_version="scc-v0",
        model=model,
        document_id=f"scc:{task_id}",
        report_type="scc",
        metadata=ReportMetadata(
            title=f"{company_name} — EU SCC 审查报告草案",
