...
from backend.domains.eu.dpia.schema import DPIAAsyncAccepted, DPIAAsyncStatus, DPIARequest, DPIAResult
...
@router.post("/generate", response_model=DPIAResult, dependencies=[Depends(require_healthy_llm)])
def generate_dpia(
    payload: DPIARequest,
...
@router.post("/generate_async", response_model=DPIAAsyncAccepted, dependencies=[Depends(require_healthy_llm)])
...
@router.post(
    "/tasks/{task_id}/retry",
    response_model=DPIAAsyncStatus,
