...
Architecture (per docs/archive/design-provenance/认证标准合同路径.md Section 11):
  用户输入/上传材料
    → [资料解析层]
    → [路径诊断 Agent]  (P0)
    → [专项审查 Agent 组]  (P0+P1)
...
from backend.domains.cn.scc_review.agents import create_scc_agents, SCCAgentBase
from backend.domains.cn.scc_review.rule_engine import evaluate_hard_thresholds, determine_path
from backend.domains.cn.scc_review.fact_builder import build_scc_facts
from backend.domains.cn.scc_review.issue_builder import build_scc_issues
from backend.domains.cn.scc_review.evidence_builder import build_scc_evidence
...
TEMPLATE_PATH = report_template_path("cn", "3.1_scc_review_template_v0.docx")
TEMPLATE_MD = report_template_path("cn", "3.1_scc_review_template_v0.md")
