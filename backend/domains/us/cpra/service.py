...
TEMPLATE_PATH = report_template_path("us", "4.2_cpra_panorama_template_v0.docx")
TEMPLATE_MD = report_template_path("us", "4.2_cpra_panorama_template_v0.md")
...
class CPRAService:
    ...
    def generate_report(
        ...
        *,
        ...
        try:
            ...
            citation_bundle = self._attach_gap_citations(gap_items, citation_registry)
            ...
            chapters = self._generate_chapters_from_context(
                context_block, risk_level, all_citations, citation_registry, citation_bundle,
            )
    ...
    def _attach_gap_citations(
        ...
        regulations_by_issue: dict[str, list[dict]] = {}
        gap_issue_ids: dict[int, str] = {}

        for idx, gap in enumerate(high_medium_gaps[:8]):
            issue_id = self._gap_issue_id(gap, idx)
            gap_issue_ids[id(gap)] = issue_id
            issues.append(self._gap_to_module_issue(gap, issue_id))
            ...
            if not hits:
                ...
                hits = domain_cache.get(gap.domain, [])
            regulations_by_issue[issue_id] = hits[:3]

        for idx, gap in enumerate(gap_items):
            if gap.risk_level in ("HIGH", "MEDIUM"):
                continue
            issue_id = self._gap_issue_id(gap, idx + 100)
            gap_issue_ids[id(gap)] = issue_id
            issues.append(self._gap_to_module_issue(gap, issue_id))
            if gap.domain not in domain_cache:
                domain_cache[gap.domain] = self.legal_retriever.retrieve(gap.domain)
            regulations_by_issue[issue_id] = domain_cache.get(gap.domain, [])[:2]
        ...
        for gap in gap_items:
            ...
            if not issue_id:
                ...
                gap.citation_refs = []
                continue
            citation_ids = bundle.citation_ids_by_issue.get(issue_id, [])
            gap.citations = citation_ids
            gap.citation_refs = [refs_by_id[cid] for cid in citation_ids if cid in refs_by_id]
    ...
    @staticmethod
    def _gap_issue_id(gap: CPRAGapItem, index: int) -> str:
        return f"CPRA-{index:03d}-{gap.domain}"

    @staticmethod
    def _gap_to_module_issue(gap: CPRAGapItem, issue_id: str) -> ModuleIssue:
        category_map = {
            "opt_out": "opt_out",
            "spi": "spi_risk",
            "spi_review": "spi_risk",
            "vendor_review": "vendor_contract",
            "vendor_management": "vendor_contract",
            "dark_patterns": "dark_pattern",
        }
        return ModuleIssue(
            issue_id=issue_id,
            category=category_map.get(gap.domain, "other"),
            title=gap.gap,
            description=" ".join(part for part in [gap.gap, gap.legal_basis, gap.recommendation] if part),
            severity=gap.risk_level,
            legal_basis=gap.legal_basis,
            recommendation=gap.recommendation,
            facts=[gap.evidence_source],
        )
