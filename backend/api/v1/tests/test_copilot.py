...
from backend.api.v1.endpoints.copilot import router
from backend.core.container import AppContainer
from backend.core.settings import Settings


def test_copilot_chat_returns_local_reply_when_llm_is_disabled(tmp_path: Path) -> None:
    app = FastAPI()
    app.include_router(router, prefix="/api/v1/copilot")

    settings = Settings(storage_dir=tmp_path / "storage", tencent_api_key=None)
...
def test_copilot_chat_small_talk_stays_conversational_when_llm_is_disabled(tmp_path: Path) -> None:
    app = FastAPI()
    app.include_router(router, prefix="/api/v1/copilot")
