...
class RulebookLoader:
    ...
    def __init__(self, rulebook_path: Path | None = None) -> None:
        if rulebook_path is None:
            rulebook_path = Path(__file__).resolve().parents[2] / "data" / "review_rulebook.json"
    ...
    def get_required_groups(self, clause_type: str) -> list[list[str]]:
        return list(self.get_clause_config(clause_type).get("required_groups", []))
    ...
    def get_required_clause_types(self, document_type: str) -> list[str]:
        return list(self.get_checklist(document_type).get("required_clause_types", []))

    def get_recommended_clause_types(self, document_type: str) -> list[str]:
        return list(self.get_checklist(document_type).get("recommended_clause_types", []))

    def get_specialized_reviewer(self, document_type: str) -> str | None:
        return self.get_checklist(document_type).get("specialized_reviewer")
    ...
    def get_priority(self, clause_type: str) -> int:
        return self.get_clause_config(clause_type).get("priority", 1)
    ...
    def get_document_applicability(self, clause_type: str) -> list[str]:
        return list(self.get_clause_config(clause_type).get("document_applicability", []))
