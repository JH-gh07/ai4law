...
本文件主体审计对象是 `tmp/` 下 **2026-08-07 09:56 左右生成的离线快照**，不是当前工作区最新一次运行。后续章节加入了 2026-08-10 的知识库索引修复结果，因此不能把全文直接理解为同一时间点的系统状态。
...
> **审计总结（2026-08-07 快照）**：11个模块、98个文件全覆盖，共记录 40 项问题。该统计是历史基线，不是当前代码状态；14117、cn_flow、索引和引用链已有后续修复，其他模块需用最新运行重新判定。
...
> 依据来源：
> - `resources/new/数规通功能路径描述/` 中每任务"功能说明与路径描述"文档的 **"系统输出"** 章节
> - `resources/new/数规通功能路径描述/` 中每任务"测试案例及预期输出"文档的 **"预期输出"** 章节
...
## 十四、业务需求差距分析：需求文档规定的预期产出 vs 实际运行结果

> 数据来源：`resources/new/数规通功能路径描述（含reference）、流程描述、测试案例/` 下每个任务的 **"系统输出"段落** + **"测试案例及预期输出"文档**，对照 `tmp/` 下实际生成的 `_result.json` 和渲染文件。
>
> 本节回答一个问题：**业务需求要什么 vs 代码实际给了什么，差距在哪**。

---

### 14.0 需求中的10个任务与代码中11个模块的映射
...
### 15.3 CN 索引：法律覆盖全面，但模块分配失衡且内容过短
...
## 十七、2026-08-10 全模块重新运行交叉验核

> 验核时间：2026-08-10 13:16-13:18（最新任务运行）
> 验核方法：取每个模块 outputs/ 下最新 UUID 任务目录，逐项比对审计报告 40 个问题
> 验核对象：assessment、pipia、dpia、us_14117、bcr、eu_scc、tia、cpra（8 模块有 outputs/ 产物）
...
### 17.2 跨模块脚注覆盖率排行（2026-08-10 最新运行）
...
### 17.4 evidence_chain 三字段填充率（C-COMMON-3 详细）
...
### 17.5 PIPIA 专项：从 CIT 截断到 LLM 未启动

审计报告（2026-08-07）将 pipia 问题定位为 CIT 标记截断。2026-08-10 最新运行结果：
