#!/usr/bin/env python3
"""Product RAG retrieval benchmark with source-level labels and robust metrics."""
...
DATASET_CSV = RETRIEVAL_DATASET_DIR / "queries.csv"
EXTRA_NEGATIVE_CSV = RETRIEVAL_DATASET_DIR / "hard_negatives.csv"
SOURCES_CSV = sources_csv_path()
OUT_JSON = ROOT / "outputs/benchmarks/rag_retrieval.json"
OUT_MD = ROOT / "outputs/benchmarks/rag_retrieval.md"
...
def _load_rows() -> list[QueryRow]:
    ...
    if not rows:
        raise ValueError("RAG retrieval benchmark dataset is empty")
    query_ids = [row.query_id for row in rows]
    duplicates = sorted({query_id for query_id in query_ids if query_ids.count(query_id) > 1})
    if duplicates:
        raise ValueError(f"Duplicate query_id: {', '.join(duplicates)}")
    invalid_labels = sorted({row.label_type for row in rows if row.label_type not in {"positive", "negative"}})
    if invalid_labels:
        raise ValueError(f"Unsupported label_type: {', '.join(invalid_labels)}")
    return rows
