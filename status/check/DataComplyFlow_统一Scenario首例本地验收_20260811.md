...
> 范围：阶段 3 的 EO 14117 三色纵向切片；仅本地，未连接远程，未部署
...
benchmarks/cases/us_14117/geneguard_geolocation_yellow/
  scenario.json
  expected.json
benchmarks/cases/us_14117/geneguard_telemetry_green/
  scenario.json
  expected.json
```
...
前端测试要求三个 builder 生成的完整 payload 与各自 `scenario.request` 深度相等。CLI 的红、黄、绿三个案例已删除内联 `input` 和断言，分别只保留 `scenario_path`、`expected_path`。
...
共享绿灯 CLI：PASS，22/22 断言，Traffic light=GREEN，rule_hits=0
llm_calls=0，tokens=0；每条案例 Trace=25 个事件
前端共享 payload 测试：20/20 passed
前端全量测试：150 passed，2 skipped；build 通过
后端相关测试：92 passed
CLI 全量：22 PASS / 0 FAIL，468 leaf checks
前端 build：通过
...
2d4b5ca feat(cases): share EO 14117 green scenario
89ad8b6 test(cases): expect EO three-color coverage
```

## 尚未完成

- EO 14117 红灯、黄灯、绿灯已实现前端与 CLI 事实及预期同源；其他模块仍未迁移。
- 三色 CLI 强断言均已从案例壳迁移到各自 `expected.json`，门禁和 runner 均读取共享预期。
- `cn_flow` 已完成三色共享 scenario 的核心事实和灯号等价测试，但旧结构丢失字段仍会列入 `lossy_fields`。
- 本地 HTTP、SSE 和浏览器验收尚未执行。

## 结论

EO 14117 三色案例已经完成共享场景闭环，但阶段 3 的其他模块仍未完成。主方案继续保留在 `status/todo/`。
