...
> 最新结论（2026-08-09 23:47）：**TIA 本地完整闭环通过。最终 Schema-first live 任务由确定性规则判定“暂停传输”，真实解析 EDPB/FISA/CLOUD Act 材料；Markdown、DOCX、PDF、ZIP、DocumentIR、CitationMap 均生成；Markdown/CitationMap/DocumentIR 的 GDPR 44/46 与 EDPB Step 1/3 引用身份一致；浏览器引用精确跳转和 PDF 预览通过。远端部署仍未执行。**
>
> 阅读说明：第 1—22 节保留历次失败、修复和阶段结论，不能单独当作当前状态；**最终状态以第 23 节为准**。
...
因此本报告的准确结论是：**Schema-first 开关、DocumentIR 生成、Compiler 成功门禁、旧输出兼容性、本地文件产出、结构化规则、本地 PDF 解析、DPO 问题传播和法规条文语义定位已经通过；live 执行、浏览器展示和技术层跳转已通过。尚未通过的是修复索引后 live 任务的 `ART46` 引用复验，以及该测试输入本身的 DPO 质量审核。**
...
| 优先级 | 未完成项 | 原因 | 验收方式 |
|---|---|---|---|
| P0 | 修复索引后的 live `ART46` 引用复验 | Schema-first live 已完成，但运行发生在 RAG 哈希刷新修复之前 | 新任务 CitationMap 必须包含 `CIT-EU-GDPR-ART46-P01`，正文相关主张使用该引用并可跳转 |
| P0 | DPO 发现的输入/报告缺口 | live 已正确传播 `needs_revision`，但案例未提供明确角色及完整美国法律证据 | 补齐案例证据后，DPO 返回 approved；不得通过隐藏问题或改写风险等级绕过 |
...
| Schema-first live 产物 | **通过（CLI）** | 第 19 节：DocumentIR 生成、Compiler 成功、16/16 checks |
| DPO 问题传播 | **通过（CLI）** | 第 19 节：两条问题进入 result 和 final_brief，next_steps 为修复后重审 |
| 当前案例 DPO 审核结论 | **不通过** | `needs_revision`；角色和美国法律证据不足，报告不能作为已审核通过产物 |
| 异步产物侧栏断链 | 已修复 | 提交 `aecf75f`，截图 07、08，前端 128/2 |
| 远端部署 | 未执行 | 本地限制仍有效 |

最终结论：**TIA 的本地执行链路、Schema-first live 和法规条文语义定位已跑通，但当前案例仍不是可交付的合规报告。下一步补齐测试输入的角色/第三国法律证据，并用已刷新索引重跑 live 浏览器任务，验证 `ART46` 与 DPO approved；完成前不进入远端部署。**
...
## 19. 2026-08-09 补验：Schema-first live 与 RAG 索引刷新

### 19.1 Schema-first live 运行

命令：

```bash
AI4LAW_SCHEMA_FIRST_TIA_ENABLED=true \
  uv run python backend/tests/harness/runner.py tia 02_structured_local_attachment
```

运行目录：`runs/tia/20260809_152526_171399_02_structured_local_attachment/`。

| 指标 | 实际值 |
|---|---:|
| Harness | PASS，16/16 checks，0 failed，0 skipped |
| 完整耗时 | 394,409 ms |
| LLM 调用 / trace 事件 | 9 / 25 |
| Prompt / completion / total token | 6,250 / 5,287 / 11,537 |
| Provider fallback / error | 0 / 0 |
| 用户产物 | MD、DOCX、PDF、ZIP |
| 内部产物 | DocumentIR、CitationMap |
| DocumentIR | schema 3.0，6 sections，76 blocks，Compiler 成功 |

关键文件哈希：

```text
result.json       b3abd661011f175afec758fd0b20665d630177aff95bd05b5f12b9c005f46902
run_manifest.json fdfacad3cf7be952608f90476a7056f3a5551dd4b5613dd1cd5a64264c83b682
document_ir.json  f414a12df0114bc75faf6fdb6cd4fccdd47c4904da383dd63c0f872ce84f3a76
citation_map.json ef9f9e8db9c7ca6c0e8aa11d74f12a20ef4e957bda13323809853fe0fb243646
```

### 19.2 DPO 状态的准确解释

本次任务执行状态是 `PASS/COMPLETED`，表示程序和产物生成成功；计算风险等级仍为 `LOW`；DPO 审核状态是 `needs_revision`。三者不是同一个含义。

DPO 的两条关键问题已进入：

- `TIAResult.consistency_issues`；
- trace `final_brief.detail.risks`；
- trace `final_brief.detail.next_steps = ["修复 DPO 复核问题", "重新审阅 TIA 结论"]`。

因此“问题传播断链”已修复，但测试输入/报告质量本身没有通过。报告不得因为任务完成或风险计算为 LOW 就标记为 DPO approved。

### 19.3 live 运行发现的旧索引问题

本次 live 在 RAG 哈希刷新修复之前启动。CitationMap 只包含 `CIT-EU-EDPB-ART5-P01`，没有错误的 GDPR“段落1”，但也没有召回新的 GDPR 第 44/46 条。根因是 `storage/rag/v3` 的 7 月索引只校验 schema/version/dimension，不校验法规源内容，法规 JSONL 更新后仍被判断为最新。

提交 `b10f52e` 已修复：

- 单索引写入并校验法规 JSONL 内容哈希；
- v3 法律索引写入并校验法规 JSONL、sources.csv、source registry 的组合哈希；
- 源内容变化时首次检索自动重建，不需要人工删除缓存；
- 非法律索引不因无关法规变化而被判过期。

回归结果：RAG、citation、TIA、SCC、CPRA、知识 API 共 `229 passed`；Ruff 通过。修复后的真实本地检索在 1,513 ms 内自动重建并召回：

```text
EU-LAW-001 | 44 | GDPR (EU) 2016/679
EU-LAW-001 | 46 | GDPR (EU) 2016/679 | appropriate safeguards
```

本节没有宣称 live `ART46` 已通过：live 运行早于索引刷新提交。仍需用补齐证据的案例运行一次新的本地任务，并核对正文、CitationMap、DocumentIR 和浏览器跳转四层一致。

## 20. 2026-08-09 补验：角色、真实法律材料与条文身份

提交 `5319b1a` 和 `62b2d99` 完成以下修复：

- `TIAStructuredInput` 增加出口方/进口方 GDPR 角色；OpenAPI TypeScript 类型同步生成；
- `US/USA` 统一映射到风险簿 `United States`，国家风险从未知 `MEDIUM` 修正为 `HIGH`，并带入 FISA 702、EO 12333、CLOUD Act；
- 案例明确为 controller→processor，并真实读取 EDPB、FISA Section 702、CLOUD Act 三份本地 PDF；
- TIA 法规去重从仅使用 `source_id` 改为 `(source_id, article)`，同一法规第 44/46 条不再互相覆盖；
- EDPB 从 6 个标题/正文伪“段落”重建为 `Step 1`、`Step 3` 两个语义定位；
- `sources.csv` 明确 GDPR 与 EDPB 为 TIA/SCC/BCR 共用来源，source registry 由权威 CSV 重建并通过零差异检查；
- SCC 对 EU 2021/914 的必需法律依据增加窄查询，避免被长查询排序挤出候选集。

法规完整性门禁结果：102 个来源，3,026 条记录，3,026 条唯一可解析，0 重复，0 缺失定位符，100% resolution success。联合回归覆盖 TIA、SCC、BCR、CPRA、RAG、citation、knowledge API，测试均运行至 100%；Ruff 和 source registry 同步检查通过。

浏览器知识页复验：

```text
http://127.0.0.1:5173/knowledge/laws/EU-GUIDE-002?article=Step%203
```

页面高亮 `Step 3 / Current Article`，正文为 `Assess transfer tool effectiveness` 及 Article 46 transfer tool 实效评估内容；页面不含“段落1”，console error/warning 为 0。

![EDPB Step 3 语义跳转修复](browser/11_edpb_step3_semantic_jump_fixed.png)

## 21. 2026-08-09 live 截断失败与确定性重放

修复条文身份后的 live 运行：

```text
runs/tia/20260809_175321_378066_02_structured_local_attachment/
```

实际执行了 9 次 LLM 调用、23 个 trace 事件、14,256 token，0 provider fallback；六章均已生成。任务最终 `FAIL`，原因不是业务判断或引用缺失，而是第六章在 token 边界被截断，末尾残留未闭合 `**`，Schema-first `ParagraphBlock` 按规则拒绝 Markdown 标记，因此没有生成用户产物。Harness 的 8 项失败均是结果为空后的连带失败。

提交 `309d876` 在共享 `strip_markdown_inline()` 中清理段落末尾未闭合的 `**`、`***`、`__`、`___` 或反引号；不改变正文、不删除引用 marker。TIA、DPIA、安全评估及共享 LLM 后处理共 44 项回归通过。

随后使用同一次失败运行的六章 live 原文进行本地确定性重放，不再调用模型：

| 项目 | 重放结果 |
|---|---:|
| Compiler | `success` |
| diagnostics | 0 |
| sections / blocks | 6 / 56 |
| 截断 Markdown 残片 | 0 |
| 引用 | GDPR 44、GDPR 46、EDPB Step 1、EDPB Step 3 |

这证明同一份真实 live 内容在修复后可以进入 DocumentIR，且 `CIT-EU-GDPR-ART46-P01` 和 `CIT-EU-EDPB-ARTSTEP_3-P01` 均被保留。严谨边界是：该次完整 harness 仍记录为 FAIL，不能改写为 PASS；确定性重放证明已修复具体异常，但修复后的完整 live/browser 任务仍需后续复验。

## 22. 2026-08-09 最终本地 live 验收
...
运行目录：`runs/tia/20260809_180810_138272_02_structured_local_attachment/`。

| 指标 | 实际结果 |
|---|---:|
| Harness | **PASS，16/16 checks** |
| 耗时 | 319,691 ms |
| LLM 调用 / trace 事件 | 9 / 25 |
| Prompt / completion / total token | 8,575 / 5,909 / 14,484 |
| fallback / error | 0 / 0 |
| 用户产物 | Markdown、DOCX、PDF、ZIP |
| 内部产物 | DocumentIR、CitationMap |
| DocumentIR | schema 3.0，6 sections，77 blocks，diagnostics=0 |
| 附件解析 | EDPB、FISA 702、CLOUD Act 均有文本；无 parse skipped |

CitationMap 和正文已经形成四层闭环：

| 引用 | 定位符 | 正文/IR | 知识库地址 | 可跳转 |
|---|---|---|---|---:|
| `CIT-EU-GDPR-ART46-P01` | `46` | 有 | `/knowledge/laws/EU-LAW-001?article=46` | 是 |
| `CIT-EU-GDPR-ART44-P01` | `44` | 有 | `/knowledge/laws/EU-LAW-001?article=44` | 是 |
| `CIT-EU-EDPB-ARTSTEP_1-P01` | `Step 1` | 有 | `/knowledge/laws/EU-GUIDE-002?article=Step+1` | 是 |
| `CIT-EU-EDPB-ARTSTEP_3-P01` | `Step 3` | 有 | `/knowledge/laws/EU-GUIDE-002?article=Step+3` | 是 |

本次 `result.json` 的 `consistency_issues=[]`，表示确定性规则没有发现输入格式冲突；DPO 最终复核仍返回 `needs_revision`，并指出：报告在评估美国 HIGH 风险、承认补充措施无法完全消除国家安全访问风险后，仍写“有条件维持”，属于风险结论弱化。最终 brief 的下一步是“修复 DPO 复核问题、重新审阅 TIA 结论”。

所以 TIA 的最终本地判定是：

> **执行、真实附件解析、Schema-first、DocumentIR、正文引用、CitationMap、知识库跳转全部通过；DPO 质量门禁不通过，当前案例不能标记为已审核通过的合规报告。**

这不是程序故障。程序正确阻止了“任务完成 = 合规通过”的错误等价。下一步应修改案例的最终结论/补充措施证据，使其与 HIGH effective risk 一致，或提供能达到 essentially equivalent protection 的完整技术、合同和组织证据，再重新 live 验收。

## 23. 2026-08-09 最终收口：确定性暂停结论、Markdown 门禁与浏览器闭环

### 23.1 最终判定

本节替代前文“仍需再次 live/browser 复验”和“DPO 质量门禁未通过”的阶段结论。

最终案例没有把“模型生成完成”当作“允许传输”。系统按结构化风险、措施充分性、附件证据和一致性问题计算出：

```text
transfer_status: suspend
inherent_risk: HIGH
...
缺失证据为“签署或适用的传输工具文件”和“技术控制实施证据”；强制条件明确要求问题修复并经 DPO 重新审阅前不得开始或继续传输。用户填写的暂停结论只作为输入记录，不能覆盖系统规则，但与系统结论一致。

因此，TIA 功能本地验收通过的含义是：**程序能正确识别不能放行的案例，生成完整草案和证据链，并把最终状态明确标成暂停；不代表该跨境传输获得批准。**
...
```text
runs/tia/20260809_234207_312610_02_structured_local_attachment/
```

| 验收项 | 最终实际结果 |
|---|---:|
| Harness | **PASS，24/24 checks，0 failed，0 skipped** |
| 耗时 | 332,388 ms |
| LLM 调用 / trace 事件 | 9 / 25 |
| Prompt / completion / total token | 10,220 / 5,882 / 16,102 |
| Provider fallback / error | 0 / 0 |
| 用户产物 | Markdown、DOCX、PDF、ZIP |
| 内部产物 | DocumentIR、CitationMap |
| DocumentIR | schema 3.0，6 sections，45 blocks，diagnostics=[] |
| PDF | 3 页 |
| ZIP | 完整可解压，包含 Markdown、DOCX、PDF、DocumentIR |

关键文件哈希：

```text
result.json       41b82c639b57f1177b98239f2f53155b341d15ad65a846f0972727e780ff70bd
run_manifest.json f64c4594750eaf878bc5806d01f0c7b148b867783425f7149bb3d7ea8e44c280
document_ir.json  336e5ce9a3434a5fc8e3eb4175ced9e05b3114a4c098eaff8d066a1ee2b92070
citation_map.json 995de852ae2145cce2e2bc99981d65fdc50b6e2988f2edc872b9e27bca76f3a1
```
...
| 问题 | 根因 | 修复位置 | 验证 |
|---|---|---|---|
| 恢复任务的 `[1]`—`[4]` 只是文本 | 工作区 ID 与真实输出目录 ID 不同，Citation API 只按固定目录猜路径 | `backend/api/v1/endpoints/citations.py:56-87`，固定目录找不到时读取该任务已登记的 `citation_map_json` | 引用 API 11 项测试通过；浏览器按钮和跳转通过 |
| “统一、”被拆成 `## 一、` | 公共 Markdown 规范化器把词语中的“一、”误认成章节号 | `backend/common/llm/postprocess.py:_explode_packed_line`；前端同步规则 `frontend/src/lib/fallback-markdown.ts:explodePackedLine` | 后端/前端共用 15 个 Golden Cases 全通过 |
| 章节在句中截断仍进入报告 | 只清理 Markdown 标记，没有判断章节是否完整 | `backend/domains/eu/tia/deterministic_chapters.py:32-43`、`service.py:319-344` | 过短、句中截断、内部标记、不闭合标记均自动改用结构化章节；TIA 35 项测试通过 |
| `**传输工具**: ## ...` | 模板把整章 Markdown 放在行内标签后 | `backend/domains/eu/tia/report_renderer.py:34-107`、`resources/templates/eu/3.4_tia_template_v0.md` | 最终 Markdown 无行内标题、无超长假标题、标题层级稳定 |

这些修复遵循“共性下沉、差异配置、依赖单向”：公共中文 Markdown 分段规则只保留一套后端/前端契约；TIA 的业务章节门禁留在 TIA 域内；报告渲染只负责章节结构；Citation API 通过已登记产物找引用，不依赖 TIA 内部实现。

### 23.4 Markdown 和跨产物一致性
...
| 行内标签后直接出现标题 | 0 |
| 超过 120 字的异常标题 | 0 |
| `【待核验】` / `【缺少依据】` / `【证据冲突】` 内部标记 | 0 |
| `{{...}}` / TBD / TODO / undefined / null 占位 | 0 |
| 句中截断章节 | 0 |
| Markdown 脚注号 | 1、2、3、4 |
| CitationMap 脚注号 | 1、2、3、4 |
| DocumentIR citation IDs 与 CitationMap IDs | 完全一致 |

| 脚注 | 权威来源 | 精确定位 | 浏览器地址 |
|---:|---|---|---|
| 1 | EDPB Recommendations 01/2020 | Step 1 | `/knowledge/laws/EU-GUIDE-002?article=Step+1` |
| 2 | GDPR | Article 44 | `/knowledge/laws/EU-LAW-001?article=44` |
| 3 | GDPR | Article 46 | `/knowledge/laws/EU-LAW-001?article=46` |
| 4 | EDPB Recommendations 01/2020 | Step 3 | `/knowledge/laws/EU-GUIDE-002?article=Step+3` |

### 23.5 浏览器证据

浏览器使用本地测试账号和恢复任务进行复验。清空旧错误记录后重新加载，console 只有 Vite/React 开发提示，JavaScript errors 为 0。

最终报告正文显示“暂停传输”、缺失证据、强制行动，并将 `[1]`—`[4]` 渲染为可点击引用按钮：

![TIA 最终 Schema-first 报告](browser/17_tia_final_schema_first_report.png)

点击 `[4]` 后进入 `EU-GUIDE-002?article=Step+3`，页面显示 Step 3 且标记“当前查看条文”：

![TIA EDPB Step 3 精确跳转](browser/18_tia_final_edpb_step3_jump.png)

PDF 标签真实加载浏览器 PDF Viewer，显示 3 页最终报告：

![TIA 最终 PDF 预览](browser/19_tia_final_pdf_preview.png)

### 23.6 统领性验收表

| 类别 | 结果 | 证据 |
|---|---:|---|
| 输入和真实材料 | 通过 | 固定结构化案例 + EDPB/FISA/CLOUD Act 三份本地 PDF |
| 业务判断 | 通过 | 确定性规则输出 `suspend`，模型和用户结论不能覆盖 |
| RAG 和法律依据 | 通过 | GDPR 44/46、EDPB Step 1/3 均进入正文与映射 |
| Schema-first | 通过 | DocumentIR 3.0，6 sections，45 blocks，0 diagnostics |
| Token / time / trace | 通过 | 16,102 tokens，332,388 ms，25 events，9 calls |
| Markdown | 通过 | 无截断、无异常标题、无内部标记、无占位符 |
| 引用跳转 | 通过 | 浏览器 Step 3 精确定位；四项地址均为知识库精确路由 |
| 用户产物 | 通过 | Markdown、DOCX、PDF、ZIP；PDF 3 页 |
| 浏览器 | 通过 | 正文、引用按钮、法规页、PDF Viewer；新加载 errors=0 |
| 远端部署 | **未执行** | 等用户明确同意后另行部署 |

最终结论：**TIA 本地完整闭环通过，可以从“部分完成”改为“本地已完成”；当前仍只允许继续下一个本地模块，不允许远端部署，也不开始删除全部旧流程。**
