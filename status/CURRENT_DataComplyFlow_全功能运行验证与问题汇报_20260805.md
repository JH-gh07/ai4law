...
> ⚠️ **2026-08-13 事实刷新**：本文件为 2026-08-05 审计时点快照，其中「528 passed / 15 CLI 案例 / 9/26 前端直投」等数字已过期。当前最新事实：**后端 1032 passed / 3 failed；26 个 CLI 案例 / 603 条断言全部 PASS；前端案例已收敛为 28 个（前端与 CLI 同源 Scenario）**。权威最新快照见 [`view/20260813_项目最新现状与事实基线更新.md`](view/20260813_项目最新现状与事实基线更新.md)；各模块 CLI 流程与真实运行数据见 [`view/20260813_各模块CLI功能代码流程逻辑原理与数据运行真实情况.md`](view/20260813_各模块CLI功能代码流程逻辑原理与数据运行真实情况.md)。以下历史内容保留供追溯。
