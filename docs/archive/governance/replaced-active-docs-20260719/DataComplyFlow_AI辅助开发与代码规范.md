...
法律业务实现已按法域收敛到 `backend/domains/{cn,eu,us}/`，历史 `backend/modules/` 已退役。新代码必须使用 `config/module_registry.json` 中的稳定 module ID，并放入对应法域目录；公共 API 适配器位于 `backend/api/`。任何后续改名仍须保留 `/api/v0`、`/api/v1` 和前端 Adapter 的兼容测试。

## 11. 仓库资源路径规范

- 后端代码通过`backend/core/resource_paths.py`访问法律知识和报告模板；
- 禁止在Service、Router、Agent或Renderer中新增`Path("doc/...")`硬编码；
- 资源物理迁移只修改统一路径入口，并必须附带文件Hash、数量和回归结果；
- 运行输出与仓库资源分离，禁止把`outputs/`、Trace、上传文件作为源码资源；
- 缺失模板必须显式记录降级行为，不得生成空文件后宣称完整交付。
