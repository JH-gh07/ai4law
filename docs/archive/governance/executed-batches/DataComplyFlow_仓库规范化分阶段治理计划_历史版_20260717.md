...
每组必须单独核对 diff、消费者、测试和兼容性。理论研究文档已于 2026-07-17 经项目负责人确认纳入治理留存，但人工审查结束前不执行 Git 提交。完成上述收敛后，才进入阶段 C 的 v0 安全与生命周期审计；阶段 D 的 RAG fallback 语义治理紧随其后，但二者不得混成一次修改。
...
| 文档治理 | 保留唯一入口、快照标记、历史归档和最小变更门禁 | 可形成独立批次 | 理论研究基线按负责人决定纳入留存，不把研究设想视为代码事实 |
...
- 当时工作树仅保留未受跟踪的理论研究文档；该历史状态已被 2026-07-17 的负责人留存决定覆盖。
...
| 编号 | 类型 | 事实证据 | 影响 | 认定 |
|---|---|---|---|---|
| V0-SEC-001 | 身份认证 | `backend/api/v0/task_gateway/router.py` 7 个 endpoint 均未使用 `Depends(get_current_user)`；运行时依赖列表为空 | 未登录请求可上传、创建/查询/取消任务和下载产物 | 已确认，P0 |
...
`backend/domains/us/eo14117_flow_review/tests/test_service.py` 与 `test_async_api.py` 改用 pytest `tmp_path` 和 `monkeypatch`，测试模板不再写入固定 `outputs/cn_flow/_test_templates`，模块全局模板路径会在测试后恢复。
...
## 9. 结构治理收口状态（2026-07-17，覆盖早期目录判断）

本节是当前治理状态，优先于本文早期阶段表中的历史描述。

### 9.1 已完成的结构性治理

- 顶层文档只保留 `docs/`；原 `doc/` 已按文档、法规、模板、评测和研究属性迁移；
- 原顶层 `paper/` 已并入 `resources/research/`，不再与工程文档并列；
- 业务实现已物理迁入 `backend/domains/{cn,eu,us}/`，原 `backend/modules/` 已退役并由卫生检查禁止恢复；
- CN Diagnosis 已收敛到唯一规则与推理实现；会话 API 作为明确兼容层调用该实现；
- `/api/v0` 任务网关已物理归位到 `backend/api/v0/task_gateway/`，`backend/main.py` 不再装配 Router；
- `config/module_registry.json` 已成为跨前后端模块身份唯一权威源；
- LLM 公共实现位于 `backend/common/llm/`，历史 `ai_engine` 不再是活动源码；
- RAG 业务入口已收敛至 `backend/common/rag/service.py`，策略以 `multi_index`、`single_index`、`enriched_compatibility` 按职责命名；
- 法规、模板、研究材料、Benchmark 和运行产物已分别归入 `resources/`、`benchmarks/` 与 `storage/` 边界；
- 当前代码回归为后端 `374 passed`、前端 `5 passed` 且生产构建成功，仓库卫生检查通过。

### 9.2 不再继续进行的“目录整理”

结构治理至此不再为了外观整齐继续移动稳定模块，不再合并真实职责不同的 RAG 实现，不再把兼容 API 复制到主版本，也不删除来源不明的 Trace、上传件或研究资产。后续改动必须由具体缺陷、消费者迁移或评测需求触发。

### 9.3 延期到补强阶段的事项

以下事项不是目录结构治理未完成，而是独立的产品补强任务：

1. `/api/v0` 鉴权、对象所有权、路径暴露和状态持久化；
2. RAG Fallback 对空结果、能力不覆盖和异常的分类，以及 Manifest 的持久化覆盖；
3. Benchmark 固定 backend/index/config 的执行 Harness；
4. 法律 Gold 与规则时效的专家确认；
5. 前端大 Chunk 性能优化和依赖大版本升级。

这些事项不得混入结构清理 Commit。

### 9.4 当前唯一工程阻塞

当前工作树包含大规模物理迁移产生的删除、新路径和修改记录，但本执行环境对 `.git` 只有读取权限，无法完成 staging、rename 识别、分批 commit 或 tag。文件系统结构和测试基线已经收敛；正式宣告“Git 仓库治理完成”仍需在可写 Git 环境中按契约复核并形成可回滚提交。
### 9.5 人工审查与研究基线决议

项目负责人已确认 `docs/handoff/DataComplyFlow_理论前沿与DataComplyBench-CN研究基线_20260713.md` 纳入治理并留存。该文档的身份是研究与 Benchmark 支撑基线，不是产品事实、法律 Gold 或已实现能力证明。

当前进入人工代码审查冻结期：审查结束前不执行 staging、commit、tag，不继续叠加结构调整。审查发现的问题应逐项记录，按“问题事实—影响范围—最小修改—验证命令”形成独立候选批次。